...
- 使用类型化 Case/Step 模型加强 YAML 与 Excel schema 校验；
- 将配置上下文和用例运行上下文分层，提升隔离性；
- 使用显式关键字注册表完善扩展机制；
- 增加默认 HTTP 超时、统一异常和敏感字段脱敏；
- 完善 setup/teardown 的异常安全和资源清理；
- 增加解析器、断言、变量渲染及执行器单元测试；
- 引入 `pyproject.toml`、代码质量检查、覆盖率和 CI；
- 在状态隔离完成后补充 pytest-xdist 并行验证；
- 增加更多 HTTP 方法、文件上传、接口 Schema 校验和页面操作关键字。
